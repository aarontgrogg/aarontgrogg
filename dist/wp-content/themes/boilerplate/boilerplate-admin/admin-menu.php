...
/*	3)	Add "HTML5 Boilerplate Admin" Page options */
		...
		if ( ! function_exists( 'H5BP_register_and_build_fields' ) ):
			function H5BP_register_and_build_fields() {
				...
				add_settings_field('H5BP_emoji_crap', 'WP Emoji?:', 'H5BP_emoji_crap_setting', 'boilerplate-admin', 'main_section');
		...
		//	callback fn for H5BP_emoji_crap
		if ( ! function_exists( 'H5BP_emoji_crap_setting' ) ):
			function H5BP_emoji_crap_setting() {
				$options = get_option('plugin_options');
				$checked = (isset($options['H5BP_emoji_crap']) && $options['H5BP_emoji_crap']) ? 'checked="checked" ' : '';
				echo '<input class="check-field" type="checkbox" name="plugin_options[H5BP_emoji_crap]" value="true" ' .$checked. '/>';
				echo '<p>As of <a href="https://codex.wordpress.org/Version_4.2">version 4.2</a>, WordPress automatically inserts Emoji support code into the <code>head</code> of every page.  There is no opt-in or opt-out option within the WordPress Admin screens, you just get it.  This feature adds JS and CSS to every page load.</p>';
				echo '<p>Selecting this option will remove the Emoji from all of your pages.</p>';
			}
		endif; // H5BP_emoji_crap_setting
...
/*	4)	Create functions to add above elements to pages */
		...
		//	$options['H5BP_emoji_crap']
		if ( ! function_exists( 'H5BP_emoji_crap' ) ):
			function H5BP_emoji_crap() {
				remove_action( 'wp_head', 'print_emoji_detection_script', 7 );
				remove_action( 'wp_print_styles', 'print_emoji_styles' );
			}
		endif; // H5BP_emoji_crap
...
/*	5)	Add Boilerplate options to page as requested */
		if (!is_admin() ) {
			...
			if (isset($options['H5BP_emoji_crap']) && $options['H5BP_emoji_crap']) {
				add_action( 'after_setup_theme', 'H5BP_emoji_crap');
			}

...
if ( ! function_exists( 'boilerplate_posted_on' ) ) :
	...
	function boilerplate_posted_on() {
		...
		// j = 1–31
		// d = 01–31
		printf( __( '<span class="%1$s">Posted on</span> <time class="entry-date" datetime="' .get_the_date( 'Y' ). '-' .get_the_date( 'm' ). '-' .get_the_date( 'd' ). '" itemprop="datePublished">%2$s %3$s, %4$s</time> <span class="meta-sep">by</span> %5$s', 'boilerplate' ),
			// %1$s = container class
			'meta-prep meta-prep-author',
			// %2$s = month: /yyyy/mm/
			sprintf( '<a href="%1$s" title="%2$s" rel="bookmark">%3$s</a>',
				home_url() . '/blog/' . get_the_date( 'Y' ) . '/' . get_the_date( 'm' ) . '/',
				esc_attr( 'View Archives for ' . get_the_date( 'F' ) . ' ' . get_the_date( 'Y' ) ),
				get_the_date( 'F' )
			),
			// %3$s = day: /yyyy/mm/dd/
			sprintf( '<a href="%1$s" title="%2$s" rel="bookmark">%3$s</a>',
				home_url() . '/blog/' . get_the_date( 'Y' ) . '/' . get_the_date( 'm' ) . '/' . get_the_date( 'd' ) . '/',
				esc_attr( 'View Archives for ' . get_the_date( 'F' ) . ' ' . get_the_date( 'j' ) . ' ' . get_the_date( 'Y' ) ),
				get_the_date( 'j' )
			),
			// %4$s = year: /yyyy/
			sprintf( '<a href="%1$s" title="%2$s" rel="bookmark">%3$s</a>',
				home_url() . '/blog/' . get_the_date( 'Y' ) . '/',
...
/*	Begin Boilerplate */
	// Add Admin
	if ( is_admin() ) {
		include_once( get_template_directory() . '/boilerplate-admin/admin-menu.php' );
	}
 ...
 */
	...
	function boilerplate_disable_wp_emojicons() {
		...
		add_filter( 'tiny_mce_plugins', 'boilerplate_disable_emojicons_tinymce' );
	}
	/*
	 * Adn just in case you use tiny_mce...
	 */
	function boilerplate_disable_emojicons_tinymce( $plugins ) {
		if ( is_array( $plugins ) ) {
	...
	add_action( 'init', 'boilerplate_disable_wp_emojicons' );
